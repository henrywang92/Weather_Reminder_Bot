...
# This program runs a Twitter bot that  searches for specific tweets that indicate the
# user will be going outside, then sends them a tweet that reminds them of the current
# weather forcast. It uses the Twitter API to search and post tweets, and uses the
# Open Weather Map api to get current weather forcast.
# Additionally, it uses the python-twitter (https://github.com/bear/python-twitter) wrapper
# to simplify interface with the Twitter API, and uses the Requests library
# (http://docs.python-requests.org/en/master/) to simplify API requests to the
# Open Weather Map API (https://openweathermap.org/api)

# In order to limit scope, this code only concerns itself with NYC residents who tweet
# about going outside, and also only searches for a very specific phrase, in order to
# avoid having to deal with a language parser.

# If you wish to run this script, you must also create a keys.py file in order to store the
# various API keys. It should be a five line file, formated as follows:
# twitter_consumer_key = 'xxxxx'
# twitter_consumer_secret = 'xxxxx'
# twitter_access_key = 'xxxxx'
